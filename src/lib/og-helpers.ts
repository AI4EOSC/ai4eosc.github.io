...
export function loadFont(): Buffer {
  if (!_fontBuffer) {
    _fontBuffer = readFileSync(
      join(process.cwd(), 'public/fonts/clashgrotesk/ClashGrotesk-Variable.woff')
    );
  }
  return _fontBuffer;
}
...
function radialDef(
  id: string,
  cx: number, cy: number,   // pixel coords in 1200×630 space
  rx: number, ry: number,   // pixel radii
...
function buildBgSvg(theme: ThemeKey): string {
  ...
  const T: Record<ThemeKey, ThemeCfg> = {
    teal: {
      ...
      radials: [
        ...
        [0.12*W, 0.12*H, 0.60*W, 0.55*H, '#ffffff', 0.14, 0.55],
        [0.85*W, 0.88*H, 0.55*W, 0.45*H, '#00282c', 0.45, 0.58],
        [0.55*W, 0.48*H, 0.40*W, 0.35*H, '#00c8d7', 0.12, 0.55],
      ],
      glowColor: '#00c8d7',
    },
    black: {
      ...
      radials: [
        [0.05*W, 0.50*H, 0.55*W, 0.80*H, '#008792', 0.14, 0.60],
        [0.92*W, 0.50*H, 0.40*W, 0.70*H, '#ec5d80', 0.10, 0.60],
        [0.50*W, 0.00*H, 0.60*W, 0.50*H, '#ffffff', 0.04, 0.50],
      ],
      glowColor: '#00c8d7',
    },
    blue: {
      ...
      radials: [
        ...
        [0.82*W, 0.86*H, 0.50*W, 0.42*H, '#000532', 0.50, 0.55],
      ],
      glowColor: '#6496ff',
    },
    pink: {
      ...
      radials: [
        [0.12*W, 0.12*H, 0.60*W, 0.55*H, '#ffffff', 0.13, 0.55],
        [0.85*W, 0.88*H, 0.55*W, 0.45*H, '#500a1e', 0.45, 0.58],
        [0.55*W, 0.48*H, 0.40*W, 0.35*H, '#ff78a0', 0.12, 0.55],
      ],
      glowColor: '#ec5d80',
    },
  };
  ...
  const radialDefs = cfg.radials.map(([cx, cy, rx, ry, color, op, spread], i) =>
    radialDef(`rg${i}`, cx, cy, rx, ry, color, op, spread)
  ).join('');

  const radialRects = cfg.radials.map((_, i) =>
    `<rect width="${W}" height="${H}" fill="url(#rg${i})"/>`
  ).join('');
  ...
  const circleMasks = [520, 365, 230].map(r =>
    `<mask id="em${r}">` +
...
async function satoriToPng(element: object): Promise<Buffer> {
  const svg = await satori(element as any, {
    ...
    fonts: [{
      name: 'Clash-Grotesk',
      data: loadFont() as unknown as ArrayBuffer,
      weight: 400,
      style: 'normal',
    }],
  });
...
function footerRowEl(urlColor = 'rgba(255,255,255,0.85)', dotColor = 'rgba(255,255,255,0.85)'): object {
  return {
    type: 'div',
    props: {
      ...
      children: [
        ...
        },
        {
          type: 'div',
          props: {
            style: { fontSize: '30px', fontWeight: '500', color: urlColor },
            children: 'ai4eosc.eu',
          },
        },
      ],
...
export async function renderOgPng(element: object, theme: ThemeKey = 'teal'): Promise<Buffer> {
  const [bgPng, textPng, logoPng] = await Promise.all([
    sharp(Buffer.from(buildBgSvg(theme))).png().toBuffer(),
    satoriToPng(element),
